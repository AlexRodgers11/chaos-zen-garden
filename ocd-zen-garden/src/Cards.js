...
function Cards(props) {
    ...
    const [numCards, setNumRows] = useState(7);
                ...
                <div style={{display: 'flex', flexDirection: 'column', justifyContent: 'space-between', width: '100%', height: '100%'}}>
                        {displayCards().map(cardLine => {
                            return <div style={{margin: '7% 0'}}>{cardLine.map(card => {
                                // return <div style={{display: 'inline-block', backgroundColor:`${card.color}`, border: `1px solid ${getColor('border', colorPalette)}`, width: `${props.width * .70 * (1 / (numCards + 2))}px`, height: `${1.5 * props.width * .70 * (1 / (numCards + 2))}px`, margin: `${(props.width * .70 * (1 / (numCards + 2)) / (numCards + 2))}px`}}></div>
